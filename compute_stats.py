...
def run_nuclei_type_stat(pred_dir, true_dir, type_uid_list=None, exhaustive=True):
    ...
    for file_idx, filename in enumerate(file_list[:]):
        ...
        path = './dataset/sample_tiles/pred/json'
        # pred_info = sio.loadmat(os.path.join(pred_dir, basename + ".mat"))
        with open('%s/%s.json' % (path, basename), "r") as handle:
    ...
    print('Fd:', results_list[0])
    print('Fm:', results_list[2])
    print('Fi:', results_list[3])
    print('Fe:', results_list[4])
    print('Fs:', results_list[5])
    # print(np.array(results_list))
    return
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--pred_dir", help="point to output dir", nargs="?", default="dataset/sample_tiles/pred/mat/", const=""
    )
    parser.add_argument(
        "--true_dir", help="point to ground truth dir", nargs="?", default="dataset/CoNSeP/Test/Labels", const=""
    )
